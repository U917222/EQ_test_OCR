...
本番では Cloudflare Access JWT を基本にしてください。共有パスワードは小規模検証や暫定運用向けです。
...
委譲しない場合は `web/functions/_lib/d1Backend.ts` が D1 に保存します。D1 経路は候補者、ファイル、セル、レビュー、採点、監査ログを持ちますが、OCR は走らず、`getResultPdf` もまだ未実装です。採点は `web/functions/_lib/cheqScoring.ts` の fallback rank を使うため、`RankRules` を読む GAS/Sheets 経路と完全同等とは限りません。
...
Google Sheets API は ADC を使います。Cloud Run 実行サービスアカウントに対象スプレッドシートを共有してください。
...
| `score_bands` | 点数帯と段階 |
| `rank_rules` | 総合ランク判定ルール |
| `handwritten_totals` | 手書き合計との突合用 |

### Google Sheets

GAS と `scoring-api` は Google Sheets 上の同等シートを使います。主なシートは `Candidates`、`RawCells`、`ReviewQueue`、`ItemMaster`、`ScoreBands`、`RankRules`、`HandwrittenTotals`、`Results`、`AuditLog`、`ApiOperations`、`ApiNonces`、`ApiUsers` です。
...
| ファイル | 役割 |
| --- | --- |
| `scoring-api/main.py` | HTTP 入口、`/healthz`、`/api` |
...
2. Cloud Run 実行サービスアカウントを対象 Spreadsheet に共有する。
3. `FUNCTIONS_GAS_SECRET` と `SCORING_SPREADSHEET_ID` を設定する。
4. `/healthz` と Pages Functions からの `me` action で疎通確認する。
